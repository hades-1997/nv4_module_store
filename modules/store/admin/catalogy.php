...
if ($nv_Request->isset_request('get_alias_title', 'post')) {
    ...
    $stmt = $db->prepare('SELECT COUNT(*) FROM ' . NV_PREFIXLANG . '_' . $module_data . '_catalogy WHERE alias = :alias');
    $stmt->bindParam(':alias', $alias, PDO::PARAM_STR);
    $stmt->execute();

    if ($stmt->fetchColumn()) {
        if($id>0){
            $main_alias = $db->query('SELECT alias FROM ' . NV_PREFIXLANG . '_' . $module_data . '_catalogy WHERE id=' . $id)->fetchColumn();
            $alias = $main_alias . '-' . $alias;
        }else{
            $weight = $db->query('SELECT MAX(id) FROM ' . NV_PREFIXLANG . '_' . $module_data . '_catalogy')->fetchColumn();
